/*
    First find total number of arrangements that is n!
    Then find total invalid arrangemetns 
    -> number of arrangements after fixing 1 elements
        nC1*(n-1)!
    -> number of arrangements after fixing 2 el
        nC2*(n-2)!
    similarly till nCn*(0)!
    Using inclusive-exclusive technique
*/
...
Factorial fac(1e6, MOD);
int f(int n) {
    int ans = fac.factorialOf(n);
    int invalid_permutations = 0;
    for(int i = 1 ; i <= n; i++) {
        int curr_inv_p = ((fac.binomialCoefficientOf(n, i)%MOD)*(fac.factorialOf(n-i)%MOD))%MOD;
        if(i%2) invalid_permutations = ((invalid_permutations%MOD) + (curr_inv_p%MOD))%MOD;
        else invalid_permutations = ((((invalid_permutations%MOD) - (curr_inv_p%MOD))%MOD) + MOD)%MOD;
    }
    
    return ((ans - invalid_permutations)+MOD)%MOD;
}
